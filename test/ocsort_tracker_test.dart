...
void main() {
  group('OCSORTTracker conformance', () {
    final cases = jsonDecode(fixtureJson('ocsort_cases.json')) as List<dynamic>;
      ...
      test(testCase['name'] as String, () {
        ...
        final tracker = OCSORTTracker(
          ...
          directionConsistencyWeight:
              (params['direction_consistency_weight'] as num?)?.toDouble() ??
              0.2,
          highConfDetThreshold:
              (params['high_conf_det_threshold'] as num?)?.toDouble() ?? 0.6,
          deltaT: params['delta_t'] as int? ?? 3,
        );
